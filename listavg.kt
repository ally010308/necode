...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val accelerationListX = mutableListOf<Float>()
    private val accelerationListY = mutableListOf<Float>()
    private val accelerationListZ = mutableListOf<Float>()
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        when (sensorType) {
            ...
            Sensor.TYPE_LINEAR_ACCELERATION -> {
                ...
                //리스트에 추가
                accelerationListX.add(line[0])
                accelerationListY.add(line[1])
                accelerationListZ.add(line[2])
    ...
    private fun getDiswT() {
        ...
        //리스트 평균
        nowAccX = accelerationListX.average().toFloat()
        nowAccY = accelerationListY.average().toFloat()
        nowAccZ = accelerationListZ.average().toFloat()
        ...
        tvLineZ!!.text = resources.getString(R.string.label_lineZ, nowAccZ)
        ...
        //리스트 초기화
        accelerationListX.clear()
        accelerationListY.clear()
        accelerationListZ.clear()
    ...
    private fun checkSignCal(a: Float, b: Float, elapsedTime: Double): Float {
        ...
        return if ((a < 0 && b > 0) || (a > 0 && b < 0)) { //속도의 부호가 바뀔 경우 거리계산
            val aA = abs(a) //절대값으로 반환, import필요
            val bA = abs(b)
            val c= aA+bA
